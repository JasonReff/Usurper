...
public class EnemyAIManager : CharacterManager
{
    ...
    private void GetFirstPossibleBoard(List<VirtualBoard> virtualBoards)
    {
        ...
        var unit = Board.Instance.GetTileAtPosition(chosenBoard.Move.CurrentTile.TilePosition()).Unit;
            ...
            Board.Instance.GetTileAtPosition(chosenBoard.Move.NewTile.TilePosition()), Board.Instance))
        {
            ...
            unit = Board.Instance.GetTileAtPosition(chosenBoard.Move.CurrentTile.TilePosition()).Unit;
        }
    ...
    private VirtualBoard? PossibleCheckmate(List<VirtualBoard> boards)
    {
        ...
        var unit = Board.Instance.GetTileAtPosition(board.Move.CurrentTile.TilePosition()).Unit;
    ...
    {
        var unit = Board.Instance.GetTileAtPosition(_chosenMove.CurrentTile.TilePosition()).Unit;
        ...
        Debug.Log($"Unit present at position: {_chosenMove.NewTile.Unit}");
        //OnUnitMoved?.Invoke();
    }
    ...
    private void GetBestUnitPlacement()
    {
        ...
        var numberOfPawns = Board.Instance.EnemyUnits.Where(t => t.GetType() == typeof(PawnUnit) || t.GetType() == typeof(ShogiPawnUnit)).Count();
    ...
    private VirtualBoard GetFirstPossiblePlacementBoard(List<VirtualBoard> boards)
    {
        ...
        var chosenBoard = virtualBoards.First();
        while (Board.Instance.GetTileAtPosition(chosenBoard.Move.NewTile.TilePosition()).Unit != null)
        {
            virtualBoards.Remove(chosenBoard);
            chosenBoard = virtualBoards.First();
        }
